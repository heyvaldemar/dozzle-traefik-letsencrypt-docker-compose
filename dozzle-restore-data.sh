#!/usr/bin/env bash
# dozzle-restore-data.sh [backup-file-name]
#
# Replaces Dozzle's application data with one of the archives the backups
# service wrote.
#
#   ./dozzle-restore-data.sh               list and ask
#   ./dozzle-restore-data.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than dozzle.
set -Eeuo pipefail
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Dozzle." >&2
  exit 1
fi
