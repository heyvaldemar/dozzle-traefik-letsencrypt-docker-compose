#!/bin/bash
# End-to-end tests for the dozzle-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# Dozzle stores no logs - it streams them from the daemon on every page load -
# so what is in /data is small and entirely irreplaceable: the user file that
# is the only thing standing between the internet and every log line on this
# host, and each user's interface settings. The test that matters is whether a
# file written before a backup comes back after a restore that has since
# destroyed it.
#
...
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-dozzle-traefik-letsencrypt-docker-compose.yml}"
...
: "${DATA_BACKUPS_PATH:=/srv/dozzle-data/backups}"
: "${DATA_BACKUP_NAME:=dozzle-data-backup}"
: "${DATA_PATH:=/data}"
: "${BACKUP_INTERVAL:=24h}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq dozzle | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: dozzle container not found" >&2; exit 1; }
...
test_env_required() {
  grep -qE '^DOZZLE_HOSTNAME=.' .env || { echo "  DOZZLE_HOSTNAME is not set in .env" >&2; return 1; }
...
test_backup_carries_the_user_file() {
  # A tarball of /data that happens to contain a directory proves very little.
  # users.yml is the accounts: lose it and the next start has no way to
  # authenticate anyone, against a service whose whole job is showing every log
  # line on the host.
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^data/users.yml'" \
    || { echo "  $f does not carry users.yml" >&2; return 1; }
  echo "  the archive carries the user file"
}
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./dozzle-restore-data.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./dozzle-restore-data.sh failed" >&2; return 1; }
...
run_test test_backup_carries_the_user_file
